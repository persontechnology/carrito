@extends('layouts.app',['title'=>'Nueva factura'])
@section('breadcrumbs', Breadcrumbs::render('nuevaFactura',$tipo))
@section('content')


    
    
    <div class="alert bg-danger text-white alert-styled-left alert-dismissible fade show bor" id="alertaErrores" style="display: none;" role="alert">
            ...
            <div class="card">
                <div class="card-header">
                    <div class="btn-group" role="group" aria-label="Basic example" id="">
                        <a href="{{ route('nuevaFactura',$tipo) }}" class="btn btn-dark">
                            <i class="fas fa-cart-plus fa-2x"></i> Nuevo
                        </a>
                ...
                <div class="card">
                    ...
                    <div class="card-header bg-transparent">
                        <div class="row">
                            ...
                            <div class="md-form input-group">
                                ...
                                <div class="input-group-append" id="MaterialButton-addon4">
                                    ...
                                    <i class="fa fa-search mr-1" aria-hidden="true"></i>Buscar
                                    </button>
                                    <button class="btn btn-md btn-primary m-0 px-3" type="button" data-toggle="modal" data-target="#modalListadoClientes">
                                    <i class="fas fa-user-check mr-1"></i> Cambiar
                                    </button>
                                </div>
                            </div>
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-6">
                                <div class="md-form md-outline my-0">
                                    <input id="documento_referencia"  class="form-control " type="text" value="{{ old('documento_referencia') }}" name="documento_referencia" required>
                                    <label for="documento_referencia" class="active"><strong>Documento referencia</strong></label>
                            ...
                            <div class="col-md-6">
                                <div class="md-form md-outline my-0">
                                    <input id="codigo"  class="form-control " type="text" value="{{ old('codigo') }}" name="codigo" required>
                                    <label for="codigo" class="active"><strong>Código</strong></label>   
                                </div>
                                    ...
                                    <tr>
                                        ...
                                        <td colspan="3">
                                            ...
                                            <p>
                                                ...
                                                DESCUENTO <strong class="float-right" id="descuento_factura">0.00</strong><br>
                    ...
                    <div class="card-footer">
                        <button class="btn btn-dark btn-block btn-lg" type="submit">
                            <i class="fas fa-dollar-sign fa-2x"></i> Procesar {{ $tipo }}
                        </button>
                    </div>
                </div>
            </form>
    ...
    <div class="modal fade" id="modalFactura" tabindex="-1" role="dialog" aria-labelledby="myModalLabel"
        ...
        <div class="modal-content">
            ...
            <h4 class="modal-title w-100" id="myModalLabel">
                <strong id="tituloFactura">Imprimir {{ $tipo }}</strong>
            </h4>
            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                <span aria-hidden="true">&times;</span>
            </button>
...
@prepend('linksPie')
    <script>
        ...
        function selecionarProducto(arg){
            ...
            if($('#fila_'+id_pro).length){
                ...
                if($('.v_total_fila').length<=9){
                    ...
                    var caja_v_unitario='<input type="number" name="valor_unitario['+id_pro+']" onkeyup="cambiarCantidad(this);" data-id="'+id_pro+'" id="txt_v_unitario_'+id_pro+'" value="'+$(arg).data('precio')+'" class="form-control form-control-sm">';
                    var caja_descuento='<input type="hidden" value="0" name="descuento['+id_pro+']" id="txt_valor_descuento_'+id_pro+'" class="txt_valor_descuento" /><input type="number" name="descuento['+id_pro+']" onkeyup="cambiarCantidad(this);" data-id="'+id_pro+'" id="txt_descuento_'+id_pro+'" value="0" class="form-control form-control-sm deshabilitar-descuento">';
        ...
        function cambiarCantidad(arg){
            ...
            var v_unitario=$('#txt_v_unitario_'+id).val();
            var descuento=$('#txt_descuento_'+id).val();
            $('#txt_valor_descuento_'+id).val(((v_unitario*cantidad)*(descuento/100)));
            var calculo_descuento=(v_unitario*cantidad)-(v_unitario*cantidad)*(descuento/100);
            $('#v_total_fila_'+id).html(calculo_descuento.toFixed(2));
            calcularSubTotal();
        }
        ...
        function calcularSubTotal(){
            ...
            $('.txt_valor_descuento').each(function(i,row) {
                descuento+=parseFloat($(row).val());
            });
            ...
            $('#descuento_factura').html(descuento.toFixed(2));
                            ...
                            $.ajax({
                                ...
                                success:function(data){
                                    ...
                                    if(data.success){
                                        ...
                                        $('#sub_total_factura').html('0.00');
                                        $('#iva_factura').html('0.00');
                                        $('#total_factura').html('0.00');
                                        $('#descuento_factura').html('0.00');
                                    }
    ...
    function deshabilitarDescuento(){
        @if($tipo==='Ingreso')   
         $('.txt_valor_descuento').attr("disabled", true)
         $('.deshabilitar-descuento').attr("disabled", true)
