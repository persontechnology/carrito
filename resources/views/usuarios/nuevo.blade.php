                ...
                <div class="card-body">
                    <form action="{{ route('guardarUsuario') }}" method="POST">
                        ...
                        @if (count($departamentos)>0)
                            ...
                            <select class="selectpicker show-tick" data-live-search="true" data-width="100%" name="departamento" id="departamento" title="Selecione un departamento">
                                ...
                                @foreach ($departamentos as $cat)
                                    <option value="{{ $cat->id }}" {{ old('departamento')==$cat->id?'selected':'' }} >{{ $cat->nombre }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-row">
                            <div class="col-md-6">
                                <div class="md-form md-outline my-1">
                                    <input id="banco" name="banco" class="form-control @error('banco') is-invalid @enderror" value="{{ old('banco') }}"    type="text">
                            ...
                            <div class="col-md-6">
                                <div class="md-form md-outline my-1">
                                    <input id="numero_cuenta" name="numero_cuenta" class="form-control @error('numero_cuenta') is-invalid @enderror" value="{{ old('numero_cuenta') }}"  type="text">
                            ...
                            <div class="col-md-12">
                                <div class="md-fsorm md-outline my-1">
                                    ...
                                    <select id="tipo_cuenta" name="tipo_cuenta" class="browser-default custom-select @error('tipo_cuenta') is-invalid @enderror" >
                                        <option value=""></option>
                                        <option value="Cuenta corriente" {{ old('tipo_cuenta')=='Cuenta corriente'?'selected':'' }}>Cuenta corriente</option>
                                        <option value="Cuenta con chequera" {{ old('tipo_cuenta')=='Cuenta con chequera'?'selected':'' }}>Cuenta con chequera</option>
                                        <option value="Cuenta de ahorro" {{ old('tipo_cuenta')=='Cuenta de ahorro'?'selected':'' }}>Cuenta de ahorro</option>
                                        <option value="Cuenta de nómina" {{ old('tipo_cuenta')=='Cuenta de nómina'?'selected':'' }}>Cuenta de nómina</option>
                                        <option value="Cuentas en dólares" {{ old('tipo_cuenta')=='Cuentas en dólares'?'selected':'' }}>Cuentas en dólares</option>
                                      </select>
                                    @error('tipo_cuenta')
                                        <span class="invalid-feedback" role="alert">
                                            <strong>{{ $message }}</strong>
